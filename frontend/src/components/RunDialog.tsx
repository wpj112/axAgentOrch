...
const textareaStyle: React.CSSProperties = {
  ...
  resize: 'vertical', background: '#252836', color: '#e0e0e0',
}
...
function RunDialog({ onRun, onPoll, onClose }: RunDialogProps) {
  ...
  return (
    ...
    >
      <div style={{ background: '#1a1d29', borderRadius: 12, padding: 24, width: 520, maxHeight: '80vh', overflow: 'auto', boxShadow: '0 4px 24px rgba(0,0,0,0.6)', color: '#e0e0e0' }}>
        <h3 style={{ marginTop: 0 }}>运行智能体</h3>

        <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 13 }}>
          <label style={{ cursor: 'pointer', color: mode === 'sync' ? '#60a5fa' : '#8b8fa3' }}>
            <input type="radio" checked={mode === 'sync'} onChange={() => setMode('sync')} /> 同步
          </label>
          <label style={{ cursor: 'pointer', color: mode === 'async' ? '#60a5fa' : '#8b8fa3' }}>
            <input type="radio" checked={mode === 'async'} onChange={() => setMode('async')} /> 异步
          </label>
        </div>

        <div style={{ marginBottom: 12 }}>
          <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 4, color: '#9ca3af' }}>输入内容</label>
          <textarea
            value={text}
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginBottom: 16 }}>
          <button onClick={handleClose} style={{ padding: '8px 20px', fontSize: 14, border: '1px solid #2e3345', borderRadius: 6, cursor: 'pointer', background: '#252836', color: '#e0e0e0' }}>
            关闭
          </button>
          <button
            onClick={handleRun}
            disabled={running}
            style={{ padding: '8px 20px', fontSize: 14, border: 'none', borderRadius: 6, cursor: running ? 'not-allowed' : 'pointer', background: '#3b82f6', color: '#fff', opacity: running ? 0.6 : 1 }}
          >
        ...
        {result && (
          <div>
            <div style={{ padding: '4px 12px', borderRadius: 4, fontSize: 13, fontWeight: 600, display: 'inline-block', background: result.status === 'success' ? '#1b3a1e' : result.status === 'failed' ? '#3a1b1b' : '#3a301b', color: result.status === 'success' ? '#22c55e' : result.status === 'failed' ? '#ef9a9a' : '#ffb74d' }}>
              {result.status}
            </div>
            {result.error_message && <div style={{ marginTop: 8, color: '#ef9a9a', fontSize: 13 }}>{result.error_message}</div>}
            {result.output && (
              <pre style={{ marginTop: 8, padding: 10, background: '#252836', borderRadius: 6, fontSize: 12, whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: 200, overflow: 'auto', color: '#e0e0e0' }}>
